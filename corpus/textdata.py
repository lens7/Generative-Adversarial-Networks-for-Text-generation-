...
class TextData:
    ...
    def add_pad(self, source_seq, target_len):
        target_seq = source_seq + [self.padToken] * (target_len - len(source_seq))
        return target_seq

    # def get_batch(self, samples, Q_size, A_size):
    def get_batch(self, samples, encoder_size, decoder_size):
        batch = Batch()
        ...
        for i in range(batchSize):
            ...
            assert len(batch.encoderSeqs[i]) <= encoder_size
            assert len(batch.decoderSeqs[i]) <= decoder_size

            # TODO: Should use tf batch function to automatically add padding and batch samples
            # Add padding & define weight
            batch.encoderSeqs[i]   = [self.padToken] * (encoder_size  - len(batch.encoderSeqs[i])) + batch.encoderSeqs[i]  # Left padding for the input
            batch.weights.append([1.0] * len(batch.targetSeqs[i]) + [0.0] * (decoder_size - len(batch.targetSeqs[i])))
            batch.decoderSeqs[i] = batch.decoderSeqs[i] + [self.padToken] * (decoder_size - len(batch.decoderSeqs[i]))
            batch.targetSeqs[i]  = batch.targetSeqs[i]  + [self.padToken] * (decoder_size - len(batch.targetSeqs[i]))
